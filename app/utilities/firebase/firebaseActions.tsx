...
export function addUser (roomKey: string, name:string, nickname:string){
  ...
  usersRef.push({
    [USER_KEY.NAME]: "Alex",
    [USER_KEY.NICKNAME]: "idk",
    [USER_KEY.SCORE]: 0
  }).then(snap => snap.key)
}

export function getUsersData (roomKey: string) {
  firebase.database().ref(`${roomKey}/${TREE_NODES.USERS}`).get()
    .then(data => data.val());
}

export function getSpecificUser (roomKey: string, currentUserId: string) {
  firebase.database().ref(`${roomKey}/${TREE_NODES.USERS}/${currentUserId}`).get()
    .then(data => data.val())
}

export function updateScore (roomKey: string, currentUserId:string, score:string) {
...
export function getTurn (roomKey: string) : any{
  var roomRef = firebase.database().ref(`${roomKey}`);
  roomRef.get().then(data => {return data.val().turn});
}
